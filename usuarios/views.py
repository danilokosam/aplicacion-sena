...
def usuario_list_view(request, id=None):
    ...
    if request.method == 'POST' and id:
        ...
        if form.is_valid():
            usuario = form.save()
            messages.success(
                request, f'¡El Usuario se editó de forma exitosa!')
        ...
        if form.is_valid():
            if not User.objects.filter(username=request.POST.get('documento')).exists():
                ...
                user.password = make_password(
                    "@" + request.POST['primer_nombre'][0] + request.POST['primer_apellido'][0] + request.POST['documento'][-4:])
                user.save()
            else:
            ...
            usuario.save()
            messages.success(
                request, f'¡El Usuario se agregó de forma exitosa!')
            return redirect('usuarios-listar')
        else:
